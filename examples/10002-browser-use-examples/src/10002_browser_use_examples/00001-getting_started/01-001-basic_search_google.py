...
async def main():
    ...
    task = "Task steps:" \
        ...
        " Now, the table 'Components and Modules' opens up in the middle of this page. Click 'X20 PLC' row from this table to expand it with rows for each product item of this sub-category" \
        ...
        " For each such browser tab, scroll down to the Table containing details of the product item and extract and collect the 'Basic Information' and the 'Technical Data' from each tab and add this data in tabular format in an individual markdown file, Save this markdown file in temp directory with name in the format <product item name>-<timestamped>.md ." \
        " Finally, after collecting data for all product items, close all tabs and browser. " \
        " Summarize the collected data in a tabular format with columns: Product Name, Material Number, Description and provide this summary as the final output of this task." \
